...
def test_every_v5_capability_has_requester_roles_set():
    ...
    assert set(CAPABILITIES) == expected, f"capability set drifted: {set(CAPABILITIES) ^ expected}"
    for name, cap in CAPABILITIES.items():
        assert cap.requester_roles is not None, f"{name} must have explicit requester_roles in V5"
...
def test_analyst_plus_capabilities_admit_analyst():
    for name in ANALYST_PLUS_CAPS:
        cap = CAPABILITIES[name]
        assert cap.requester_roles == frozenset({"analyst", "senior_analyst", "lab_lead"}), (
            f"{name} requester_roles drift: {cap.requester_roles}"
        )
...
def test_read_only_can_request_no_capability():
    for name in CAPABILITIES:
        assert CapabilityPolicy.permitted_requester_for(name, ["read_only"]) is False, (
...
def test_analyst_blocked_from_senior_plus_capabilities():
    for name in SENIOR_PLUS_CAPS:
        assert CapabilityPolicy.permitted_requester_for(name, ["analyst"]) is False, (
            f"analyst must not request senior+ capability {name}"
        )


def test_unknown_capability_is_fail_closed():
    assert CapabilityPolicy.permitted_requester_for("not_a_capability", ["lab_lead"]) is False


def test_empty_roles_denies():
    for name in CAPABILITIES:
        assert CapabilityPolicy.permitted_requester_for(name, []) is False, (
...
def test_helper_accepts_any_iterable():
    # tuple
    assert CapabilityPolicy.permitted_requester_for("dynamic_detonation", ("analyst",)) is True
    # set
    assert CapabilityPolicy.permitted_requester_for("dynamic_detonation", {"analyst"}) is True
    # generator
    assert (
        CapabilityPolicy.permitted_requester_for("dynamic_detonation", (r for r in ["analyst"]))
        is True
    )
