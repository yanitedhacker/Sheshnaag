...
def test_embed_specimen_persists_and_is_idempotent(app_and_session):
    ...
    with SessionLocal() as s:
        ...
        rows = (
            s.query(SpecimenBehaviorEmbedding)
...
def test_variant_diff_rejects_same_specimen(app_and_session):
    ...
    with SessionLocal() as s:
        tenant, [spec] = _seed(
            s, tenant_slug="acme-4", specs=[{"name": "x.bin", "indicators": [], "findings": []}]
        )
        svc = BehaviorSimilarityService(s)
...
def test_cross_tenant_isolation(app_and_session):
    app, SessionLocal = app_and_session
    with SessionLocal() as s:
        tenant_a, [spec_a] = _seed(
            s,
            tenant_slug="acme-5",
            specs=[
                {
                    "name": "a.bin",
                    "indicators": [{"kind": "domain", "value": "x.com"}],
                    "findings": [],
                }
            ],
        )
        tenant_b, [spec_b] = _seed(
            s,
            tenant_slug="other-5",
            specs=[
                {
...
def test_similarity_routes_embed_then_search(app_and_session):
    ...
    r = client.get(
        f"/api/v4/specimens/{sid_a}/similar", params={"tenant_id": tenant_id, "top_k": 5}
    )
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_similarity_route_404_unknown_specimen(app_and_session):
    ...
    with SessionLocal() as s:
        tenant, _ = _seed(
            s, tenant_slug="acme-7", specs=[{"name": "x", "indicators": [], "findings": []}]
        )
        tenant_id = tenant.id
    client = TestClient(app)
    ...
    with SessionLocal() as s:
        tenant, [a, b] = _seed(
            ...
            specs=[
                {
                    "name": "a",
                    "indicators": [{"kind": "domain", "value": "shared"}],
                    "findings": [],
                },
                {
                    "name": "b",
...
def test_variant_diff_route_400_on_self(app_and_session):
    ...
    with SessionLocal() as s:
        tenant, [a] = _seed(
            s, tenant_slug="acme-9", specs=[{"name": "a", "indicators": [], "findings": []}]
        )
