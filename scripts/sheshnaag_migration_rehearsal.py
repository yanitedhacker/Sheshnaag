...
def create_baseline_db(path: Path) -> None:
    ...
    sa.Table(
        ...
        metadata,
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("feed_key", sa.String(100)),
    )
    sa.Table(
        "cves",
        metadata,
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("cve_id", sa.String(80)),
    )
    sa.Table(
        "package_records",
        metadata,
    ...
    sa.Table(
        ...
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column(
            "source_feed_id", sa.Integer, sa.ForeignKey("source_feeds.id", ondelete="SET NULL")
        ),
        sa.Column("cve_id", sa.Integer, sa.ForeignKey("cves.id", ondelete="CASCADE")),
        sa.Column(
            "product_id", sa.Integer, sa.ForeignKey("product_records.id", ondelete="SET NULL")
        ),
    ...
    sa.Table(
        ...
        sa.Column(
            "product_id",
            sa.Integer,
            sa.ForeignKey("product_records.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("cve_id", sa.Integer, sa.ForeignKey("cves.id", ondelete="CASCADE")),
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            sa.text("INSERT INTO package_records (id, name) VALUES (1, 'acme-api-gateway')")
        )
        conn.execute(
            sa.text("INSERT INTO product_records (id, name) VALUES (1, 'acme-api-gateway')")
        )
        conn.execute(
            sa.text(
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="sheshnaag-migration-rehearsal-") as temp_dir:
        ...
        if downgrade.returncode != 0:
            raise RuntimeError(
                f"Alembic v4a04 downgrade failed:\nSTDOUT:\n{downgrade.stdout}\nSTDERR:\n{downgrade.stderr}"
            )
        ...
        payload = {
            ...
            "checks": {
                ...
                "maintainer_assessments_has_report_id": any(
                    col["name"] == "report_id" for col in maintainer_columns
                ),
            },
        }
