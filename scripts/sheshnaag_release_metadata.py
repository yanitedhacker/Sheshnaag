...
def main() -> int:
    ...
    parser.add_argument(
        "--output", default=None, help="Optional file path to also write the metadata JSON."
    )
    parser.add_argument(
        "--include-checks", action="store_true", help="Run lightweight audit/status checks."
    )
    parser.add_argument(
        "--test-summary", default=None, help="Optional path to a test summary JSON file."
    )
    parser.add_argument(
        "--sbom-artifact", action="append", default=[], help="SBOM artifact path to include."
    )
    args = parser.parse_args()
    git_sha = command_output(["git", "rev-parse", "HEAD"]) if shutil.which("git") else None
    git_status = (
        command_output(["git", "status", "--porcelain"], timeout=5) if shutil.which("git") else None
    )
    sbom_artifacts = [
        {
            "path": item,
            "exists": Path(item).exists(),
            "bytes": Path(item).stat().st_size if Path(item).exists() else None,
        }
    ...
    payload = {
        ...
        "tools": {
            ...
            "limactl": command_output(["limactl", "--version"])
            if shutil.which("limactl")
            else None,
        },
        "checks": {
            "python_dependency_audit": command_status(
                [sys.executable, "-m", "pip_audit", "-r", "requirements.txt"], timeout=120
            )
            if args.include_checks
            else {"status": "not_run"},
            "frontend_dependency_audit": command_status(
                ["npm", "--prefix", "frontend", "audit", "--audit-level=moderate"], timeout=120
            )
            if args.include_checks
            else {"status": "not_run"},
            "docker": {
                "available": shutil.which("docker") is not None,
                "version": command_output(["docker", "--version"])
