...
async def sync_exploits(cve_ids: list | None = None, session: Session = Depends(get_sync_session)):
    """
...
async def full_sync(
    ...
    try:
        ...
        return {"status": "completed", "message": "Full sync completed", "results": results}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get("/status")
def get_feed_status(session: Session = Depends(get_sync_session)):
    """
    ...
    # CVEs by source
    cves_by_source = session.query(CVE.source, func.count(CVE.id)).group_by(CVE.source).all()
