...
def get_top_priorities(
    ...
    priorities = aggregator.get_top_priorities(
        limit=limit, risk_level=risk_level, has_exploit=has_exploit, asset_id=asset_id
    )
...
@router.get("/summary")
def get_risk_summary(session: Session = Depends(get_sync_session)):
    """
...
@router.get("/heatmap")
def get_risk_heatmap(session: Session = Depends(get_sync_session)):
    """
...
def get_cve_risk_score(cve_id: str, session: Session = Depends(get_sync_session)):
    """
...
def get_risk_timeline(
    ...
    history = (
        ...
        .filter(RiskHistory.cve_id == cve.id)
