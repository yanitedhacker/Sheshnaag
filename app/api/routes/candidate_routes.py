...
def list_candidates(
    ...
    distro_hint: str | None = Query(None),
    kev_only: bool | None = Query(None),
    epss_min: float | None = Query(None, ge=0.0, le=1.0),
    epss_max: float | None = Query(None, ge=0.0, le=1.0),
    patch_available: bool | None = Query(None),
    exploit_available: bool | None = Query(None),
    min_observability: float | None = Query(None, ge=0.0, le=1.0),
    max_observability: float | None = Query(None, ge=0.0, le=1.0),
    assigned_to: str | None = Query(None),
    assignment_state: str | None = Query(None),
...
def assign_candidate(
    candidate_id: int, request: CandidateAssignRequest, session: Session = Depends(get_sync_session)
):
...
def defer_candidate(
    candidate_id: int, request: CandidateActionRequest, session: Session = Depends(get_sync_session)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400
            if "Cannot transition" in str(exc) or "Invalid status" in str(exc)
            else 404,
            detail=str(exc),
        ) from exc
...
def merge_candidate(
    candidate_id: int, request: CandidateMergeRequest, session: Session = Depends(get_sync_session)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400 if "Cannot" in str(exc) or "cannot" in str(exc) else 404,
            detail=str(exc),
        ) from exc
...
def recalculate_candidates(
    request: CandidateRecalculationRequest, session: Session = Depends(get_sync_session)
):
