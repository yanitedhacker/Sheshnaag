...
class AssetCreate(BaseModel):
    ...
    asset_type: str | None = Field(None, description="server, application, network_device, etc.")
    hostname: str | None = None
    ip_address: str | None = None
    environment: str | None = Field(None, description="production, staging, development")
    criticality: str = Field("medium", description="critical, high, medium, low")
    business_criticality: str | None = Field(None, description="business criticality override")
...
class VulnerabilityStatusUpdate(BaseModel):
    ...
    status: str = Field(
        ..., description="open, in_progress, patched, accepted_risk, false_positive"
    )
    notes: str | None = None
...
@router.get("/{asset_id}")
def get_asset(asset_id: int, session: Session = Depends(get_sync_session)):
...
def scan_asset(asset_id: int, session: Session = Depends(get_sync_session)):
    """
...
def get_asset_vulnerabilities(
    ...
    return service.get_asset_vulnerabilities(
        asset_id=asset_id, status=status, risk_level=risk_level
    )
...
def update_vulnerability_status(
    ...
    result = service.update_vulnerability_status(
        vulnerability_id=vulnerability_id, status=update.status, notes=update.notes
    )
