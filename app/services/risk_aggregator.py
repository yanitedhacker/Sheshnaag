...
from app.models.asset import AssetVulnerability
...
class RiskAggregator:
    ...
    def calculate_all_risks(self, limit: int | None = None) -> dict[str, Any]:
        """
        ...
        query = (
            ...
            .outerjoin(RiskScore)
            .filter(or_(RiskScore.id.is_(None), RiskScore.created_at < cutoff))
        )
        ...
        results = {"total_processed": 0, "scores_created": 0, "errors": []}
    ...
    def _update_priority_rankings(self):
        ...
        scores = self.session.query(RiskScore).order_by(desc(RiskScore.overall_score)).all()
    ...
    def get_risk_summary(self) -> dict[str, Any]:
        ...
        recent_critical = (
            self.session.query(func.count(RiskScore.id))
            .join(CVE)
            .filter(
    ...
    def get_risk_heatmap_data(self) -> dict[str, Any]:
        ...
        cvss_buckets = [(0, 4, "Low"), (4, 7, "Medium"), (7, 9, "High"), (9, 10.1, "Critical")]
