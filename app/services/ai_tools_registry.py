...
Write-side tools (``propose_detection``, ``detonate_in_sandbox``,
``run_authorized_offensive``, ``export_external``) delegate to the service
layer when DB context is available. Capability-gated tools still require a
valid authorization artifact before ``AIAgentLoop`` invokes them.
"""
...
def _ctx_session(context: dict[str, Any] | None):
    if not context:
        return None
    return context.get("session")


def _ctx_tenant_id(context: dict[str, Any] | None) -> int | None:
    if not context:
        return None
    ...
    **_: Any,
    ...
    if session is None or tenant_id is None:
        return {
            ...
            "kind": kind,
            "draft_digest": _echo_hash(draft_dict),
            "validator": {"precision": None, "recall": None, "f1": None},
            "note": "No DB context supplied; returning stub shape.",
        }

    from app.core.tenancy import resolve_tenant
    from app.services.ai_tool_actions import propose_detection as propose_detection_action

    tenant = resolve_tenant(session, tenant_id=tenant_id, default_to_demo=False)
    actor = str((_context or {}).get("actor") or "ai-agent")
    return propose_detection_action(session, tenant, kind=kind, draft=draft_dict, actor=actor)


def _detonate_in_sandbox(
    specimen_id: str | int,
    profile_id: str,
    _context: dict[str, Any] | None = None,
    **_: Any,
    ...
    from app.core.tenancy import resolve_tenant
    from app.services.ai_tool_actions import queue_specimen_detonation

    tenant = resolve_tenant(session, tenant_id=tenant_id, default_to_demo=False)
    actor = str((_context or {}).get("actor") or "ai-agent")
    return queue_specimen_detonation(
        session,
        tenant,
        specimen_id=specimen_id,
        profile_id=profile_id,
...
def _query_intel_feed(
    ...
    **_: Any,
    ...
    for ioc in iocs:
        ...
        signals = (
            session.query(ExploitSignal).filter(ExploitSignal.cve_id == cve.id).limit(10).all()
            if cve is not None
            else []
        )
        ...
        enrichment.append(
            {
                ...
                "cve": {
                    ...
                    if getattr(cve, "published_date", None)
                    else None,
                }
                if cve
                else None,
                "kev": {
                    ...
                    if getattr(kev, "date_added", None)
                    else None,
                }
                if kev
                else None,
...
def _export_external(
    ...
    **_: Any,
    ...
    if (target or "").lower() != "stix":
        base["accepted"] = False
        base["reason"] = f"target {target!r} not yet implemented — only 'stix' is wired."
        return base
    session = _ctx_session(_context)
    tenant_id = _ctx_tenant_id(_context)
    if session is None or tenant_id is None:
...
def _run_authorized_offensive(
    ...
    **_: Any,
    ...
    from app.core.tenancy import resolve_tenant
    from app.services.ai_tool_actions import queue_authorized_offensive_run

    tenant = resolve_tenant(session, tenant_id=tenant_id, default_to_demo=False)
    actor = str((_context or {}).get("actor") or "ai-agent")
    return queue_authorized_offensive_run(
        session,
        tenant,
        target=target,
        recipe_id=recipe_id,
