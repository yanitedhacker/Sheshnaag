...
class AssetService:
    ...
    def create_asset(self, asset_data: dict[str, Any]) -> Asset:
        ...
        asset = Asset(
            ...
            business_criticality=asset_data.get(
                "business_criticality", asset_data.get("criticality", "medium")
            ),
    ...
    def get_asset(self, asset_id: int) -> dict[str, Any] | None:
        ...
        vuln_counts = (
            self.session.query(AssetVulnerability.status, func.count(AssetVulnerability.id))
    ...
    def list_assets(
        ...
        for asset in assets:
            open_vulns = (
                self.session.query(func.count(AssetVulnerability.id))
                .filter(
                    AssetVulnerability.asset_id == asset.id, AssetVulnerability.status == "open"
                )
                .scalar()
            )
        ...
        for software in installed_software:
            ...
            for cve in matching_cves:
                ...
                existing = (
                    ...
                    .filter(
                        AssetVulnerability.asset_id == asset_id, AssetVulnerability.cve_id == cve.id
                    )
                    .first()
                )
    ...
    def get_asset_vulnerabilities(
        self, asset_id: int, status: str | None = None, risk_level: str | None = None
    ) -> list[dict[str, Any]]:
        """Get vulnerabilities for a specific asset."""
        query = (
            self.session.query(AssetVulnerability, CVE, RiskScore)
            .join(CVE)
            .outerjoin(RiskScore, RiskScore.cve_id == CVE.id)
            .filter(AssetVulnerability.asset_id == asset_id)
        )
    ...
    def update_vulnerability_status(
        self, vulnerability_id: int, status: str, notes: str | None = None
    ) -> dict[str, Any]:
        """Update vulnerability status."""
        vuln = (
            self.session.query(AssetVulnerability)
            .filter(AssetVulnerability.id == vulnerability_id)
            .first()
        )
    ...
    def get_organization_risk_summary(self, tenant_id: int | None = None) -> dict[str, Any]:
        ...
        # Total assets
        total_assets = self.session.query(func.count(Asset.id)).filter(*asset_filter).scalar()

        # Assets by criticality
        criticality_dist = (
            self.session.query(Asset.criticality, func.count(Asset.id))
            .filter(*asset_filter)
        ...
        vuln_by_risk = (
            self.session.query(RiskScore.risk_level, func.count(AssetVulnerability.id))
            .join(CVE, CVE.id == RiskScore.cve_id)
            .join(AssetVulnerability, AssetVulnerability.cve_id == CVE.id)
            .join(Asset, Asset.id == AssetVulnerability.asset_id)
            .filter(AssetVulnerability.status == "open", *asset_filter)
