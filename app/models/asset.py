...
class Asset(Base):
    ...
    network_exposures = relationship(
        "NetworkExposure", back_populates="asset", cascade="all, delete-orphan"
    )
    identity_principals = relationship(
        "IdentityPrincipal", back_populates="asset", cascade="all, delete-orphan"
    )
