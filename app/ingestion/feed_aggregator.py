...
from app.ingestion.connector import ConnectorResult, FeedConnector, get_registered_connectors
from app.ingestion.sync_state import (
    get_or_create_state,
    mark_failed,
    mark_running,
    mark_success,
...
class FeedAggregator:
    ...
    def __init__(self, session: Session, *, connectors: list[FeedConnector] | None = None):
        self.session = session
        if connectors is not None:
            self._connectors = {c.name: c for c in connectors}
        else:
            self._connectors = {name: cls() for name, cls in get_registered_connectors().items()}

    @property
    def connector_names(self) -> list[str]:
        return list(self._connectors.keys())

    def get_connector(self, name: str) -> FeedConnector | None:
    ...
    async def sync_connector(
        ...
        *,
        ...
        try:
            ...
            cursor: str | None = state.cursor if connector.supports_cursor else None
            if cursor:
                try:
    ...
    async def sync_recent_cves(
        self, days: int = 7, since: datetime | None = None
    ) -> dict[str, Any]:
    ...
    async def sync_recent_exploits_from_mirror(
