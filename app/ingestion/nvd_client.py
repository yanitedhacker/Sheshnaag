...
class NVDClient:
    ...
    async def fetch_cves(
        ...
        """
        params = {"resultsPerPage": min(results_per_page, 2000), "startIndex": start_index}
    ...
    async def fetch_recent_cves(
        ...
        while True:
            data = await self.fetch_cves(
                start_date=start_date, end_date=end_date, start_index=start_index
            )
    ...
    def save_cve_to_db(self, session: Session, parsed_cve: dict[str, Any]) -> CVE:
        ...
        for ref_data in parsed_cve.get("references", []):
            ref = CVEReference(
                cve_id=cve.id, url=ref_data["url"], source=ref_data["source"], tags=ref_data["tags"]
            )
            session.add(ref)
